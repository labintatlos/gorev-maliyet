...
def result_text(result: Any, fuel_type: str, light_split: DutyLightSplit, history_id: int | None = None) -> str:
    ...
    return (
        ...
        f"📊 Yakıt verimliliği: <b>{result.fuel_efficiency} L/saat</b>\n\n"
        f"🛠 Amortisman: <b>₺{money(result.amortization_cost_try)}</b> <i>({result.amortization_percentage}%)</i>\n"
        f"👥 Personel: <b>₺{money(result.personnel_cost_try)}</b> <i>({result.personnel_percentage}%)</i>\n"
        f"⛽ Yakıt maliyeti: <b>₺{money(result.fuel_cost_try)}</b> <i>({result.fuel_percentage}%)</i>\n"
        f"💰 <b>TOPLAM: ₺{money(result.total_cost_try)}</b>"
        f"\n⏱ <b>Saatlik ortalama: ₺{result.hourly_cost} TL/saat</b>"
        f"{saved}\n\n"
        "<b>GÖRRAP</b>\n"
